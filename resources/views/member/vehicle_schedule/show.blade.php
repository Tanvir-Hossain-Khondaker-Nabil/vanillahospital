            ...
            <div class="box box-primary">
                ...
                <div class="box-body">
                    <table class="table table-responsive table-striped">
                        ...
                        <tr>
                            <th colspan="1" width="250px"> Title </th>
                            <td colspan="3"> {{$lead->title}} </td>
                        </tr>
                        <tr>
                            <th colspan="1" width="250px"> Client Name </th>
                            <td colspan="3"> {{$lead->client->name}} </td>
                        </tr>
                        <tr>
                            <th>Company Name</th>
                            <td>{{$lead->clientCompany->company_name}}</td>
                            <th>Category</th>
                            <td>{{$lead->leadCategory->display_name}}</td>
                ...
                <div class="box-body">
                    <table class="table table-responsive table-striped">
                        ...
                        <tr>
                            ...
                            <tbody>
                                @foreach ( $lead->leadStatus as $row)
                                <tr>
                                    <td>{{$row->user->full_name}}</td>
                                    <td><span class="badge {{$row->lead_status =='Won'? 'bg-green': ($row->lead_status =='Canceled'? 'bg-red': '')}}">{{$row->lead_status}}</span></td>
                                    <td>{!!$row->comment!!}</td>
                                    <td>{{date_string_format_with_time($row->created_at)}}</td>
                                </tr>
