...
<div class="box">
    <div class="box-body">
        ...
        <div class="col-md-2 px-1">
            ...
            {!! Form::text('avg_purchase_price', null, ['id'=>'avg_purchase_qty', 'class'=>'form-control', 'step'=>"any", 'readonly']); !!}
        </div>
    </div>
</div>

<div class="box">
    <div class="box-body warehouse-box">
...
{{--        </div>--}}
        ...
        <div class="col-md-1">
            <a class="btn btn-info btn-sm  margin-top-23 addwarehouse" href="#" data-target="0"><i class="fa fa-industry"></i></a>
    ...
    <div class="">
        ...
        <div class="col-md-4">
            ...
            <div class="form-group">
                ...
                {!! Form::number('total_amount',0,['id'=>'total_amount','class'=>'form-control input-number','placeholder'=>'Enter Price', 'step'=>"any", 'required', 'readonly']); !!}
            </div>
            <div class="form-group">
...
<div id="all_modal">
@include('member.purchase.warehouse._model_warehouse_qty')
</div>
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            function warehouse_html($target)
            {
                var totalQty = checkLoadQty($target);

                var loadQty =  $("#warehouse_load_qty_"+$target).val();
                if(totalQty == loadQty)
                {
                    bootbox.alert(" Qty Load Completed Warehouse.");
                }else if(totalQty < loadQty) {

                    var optionName='<option selected="selected" value="">Select Name</option>';

                    $.each( $warehouses, function( key, value ) {
                        if(warehouseSelected.includes(key) === false)
                            optionName = optionName+'<option value="'+key+'" >'+value+'</option>';
                    });

                    var itemId = $("#warehouse_load_item_"+$target).val();

                    return '<tr><td width="400px"><select class="form-control select2"  name="warehouse_id['+itemId+'][]">'+optionName+'</select></td><td><input type="number" class="form-control" name="warehouse_qty['+itemId+'][]" type="number" /></td></tr>';

                }else{
                   bootbox.alert("Loaded Qty and Item qty not same Warehouse. Or any problem");
                }
            ...
            $(document).on('click', '.addwarehouse', function () {
                ...
                if ($div.find('div :first-child').find('select').val() == "" || $div.find('div :nth-child(3)').find('input').val() == "" || $div.find('div :nth-child(4)').find('input').val() == "") {
                    bootbox.alert("Please Select Item Name or input value  Qty/Price");
                    return false;
                }


                var itemId = $div.find('div :first-child').find('select :selected').val();
                var itemName = $div.find('div :first-child').find('select :selected').text();
                var qty = $div.find('div :nth-child(3)').find('input').val();
                var row = $div.find('div :first-child').find('select').data('option');

                $("#item_load_item_"+$target).html(itemName);
                $("#item_load_qty_"+$target).html(qty);
                $("#warehouse_load_item_"+$target).val(itemId);
                $("#warehouse_load_qty_"+$target).val(qty);



               var html = warehouse_html($target);

                $("#warehouse_store"+$target).append(html);

                $("#addwarehouse"+$target).modal('show');
                $('.select2').select2();
            ...
            function  checkLoadQty($target) {
                var allTr =  $("#warehouse_store"+$target).find("tr");
                var totalQty = 0;

                for (var $i=1; $i<=allTr.length; $i++)
                {
                    var warehouseId = $("#warehouse_store"+$target)
                        .find("tr:nth-child("+$i+") td:first-child select :selected").val();
                    warehouseSelected.push(warehouseId);

                    var qty = $("#warehouse_store"+$target)
                        .find("tr:nth-child("+$i+") td:last-child input").val();
            ...
            $(document).on('click', '.multiple-row', function () {
                var optionName = '';
                var $target = $(this).data('target');
            ...
            function add_row_modal_warehouse($row)
            {
               var  html = '<div id="addwarehouse'+$row+'" class="modal fade" role="dialog"><div class="modal-dialog"> <div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal">&times;</button><h4 class="modal-title">Warehouse Qty Load</h4><input type="hidden" id="warehouse_load_qty_'+$row+'" value="0"> <input type="hidden" id="warehouse_load_item_'+$row+'" value="0"></div><div class="modal-body"><h5 class="modal-title">Product Name: <span  class=" text-bold" id="item_load_item_'+$row+'"></span></h5><h5 class="modal-title">Total Qty: <span class=" text-bold" id="item_load_qty_'+$row+'"></span></h5><table class="table table-striped"><thead><tr><th> Warehouse Name</th><th> Qty</th></tr></thead><tbody id="warehouse_store'+$row+'"></tbody><tfoot><tr> <td colspan="2" > <button  id="multiple-row-'+$row+'" class="btn btn-info multiple-row" data-target="'+$row+'" type="button"> Add Row</button></td></tr></tfoot></table></div><div class="modal-footer"><button type="button" class="btn btn-success confirm_assign" data-target="'+$row+'" > Confirm Assign </button></div></div></div></div>';

               $("#all_modal").append(html);
            }
            ...
            $(document).on('click', '.add-row', function () {
                ...
                var lastTarget = $div.find('div :first-child').find('select').data('option');


                var totalQty = checkLoadQty(lastTarget);
                var loadQty =  $("#warehouse_load_qty_"+lastTarget).val();

                if(totalQty < loadQty || totalQty == 0 || loadQty == 0) {

                    bootbox.alert("Please complete you warehouse load");
                    return false;
                }
                ...
                var html = '<div class="box-body warehouse-box ">' +
                    '<div class="col-md-3">' +
                    ...
                    '{!! Form::text('description[]',null,['id'=>'description','class'=>'form-control' ,'placeholder'=>'Enter Description','height'=>'70px !important' ]); !!}' +
                    '</div>' +
                    '<div class="col-md-1"> <a class="btn btn-info btn-sm  margin-top-3 addwarehouse" href="#" data-target="'+i+'"><i class="fa fa-industry"></i></a><a class="btn btn-success add-row pull-right margin-top-3" href="#" ><i class="fa fa-plus-circle"></i> </a>' +
                    '</div>'+
                    '</div>';

                add_row_modal_warehouse(i);
            ...
            $(document).on('click','.edit-button', function() {
                ...
                var target = childrenDiv.find('select').data('option');
                $(this).parent().css("padding-top", '20px');
                var actionHtml = '<a class="btn btn-info btn-sm  margin-top-3 addwarehouse" href="#" data-target="'+target+'"><i class="fa fa-industry"></i></a><a class="btn btn-warning pull-right margin-top-3" id="edit-complete" href="#" ><i class="fa fa-check-circle"></i> </a>';
                $(this).parent('div').html(actionHtml);
            });
            ...
            function price_calculate(){
                var total_bill = 0;
                for(var i = 1; i<=$('.warehouse-box').length; i++) {
                    var qty = $('.warehouse-box:nth-child(' + i + ') div:nth-child(3) input').val();
                    // var total_price = $('.warehouse-box:nth-child(' + i + ') div:nth-child(8) input').val();

                    var price = $('.warehouse-box:nth-child(' + i + ') div:nth-child(4) input').val();
                    ...
                    // $('.warehouse-box:nth-child(' + i + ') div:nth-child(7) input').val(price);
                    $('.warehouse-box:nth-child(' + i + ') div:nth-child(5) input').val(total_price);
                }
