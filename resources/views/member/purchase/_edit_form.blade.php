...
<div class="box">
    <div class="box-body">
        ...
        <div class="col-md-4  px-1">
            ...
            {!! Form::select('supplier_id', $suppliers, null,['id'=>'supplier_id', 'class'=>'form-control select2','required', 'placeholder'=>trans('common.select_supplier_name')]); !!}
...
<div class="box">
    ...
    <div class="box-body purchase-box">
        <div class="col-md-3">
            ...
            @if($key==0) <label for="product_id">{{__('common.item_name')}} </label> @endif
            {!! Form::select('product_id[]', $products, $value->item_id,['id'=>'product_id_'.$key, 'data-option'=>$key, 'class'=>'form-control select2 item-name','required', 'placeholder'=>trans('common.select_item_name'), !$loop->last ? "readonly":'']); !!}
        </div>

        <div class="col-md-1">
            @if($key==0) <label for="amount"> {{__('common.unit')}} </label> @endif
            {!! Form::text('unit[]',$value->unit,['id'=>'unit_'.$key,'class'=>'form-control', 'readonly']); !!}
        </div>
        <div class="col-md-1">
            @if($key==0) <label for="amount"> {{__('common.last_purchase_qty')}} </label> @endif
            {!! Form::number('last_purchase_qty[]',$value->last_purchase_qty,['id'=>'last_purchase_qty_'.$key,'class'=>'form-control', 'readonly']); !!}
        </div>
        <div class="col-md-1">
            @if($key==0) <label for="amount"> {{__('common.available_stock')}} </label> @endif
        ...
        {{--</div>--}}
        <div class="col-md-1">
            @if($key==0) <label for="amount"> {{__('common.qty')}} </label> @endif
            {!! Form::number('qty[]',$value->qty,['id'=>'qty_'.$key,'class'=>'form-control qty input-number', 'step'=>"any", 'required']); !!}
        </div>
        <div class="col-md-2">
            @if($key==0) <label for="amount"> {{__('common.price')}} </label> @endif
            {!! Form::number('price[]',$value->price,['id'=>'price_'.$key,'class'=>'form-control input-number price','placeholder'=>trans('common.price'), 'step'=>"any", 'required']); !!}
        </div>
        <div class="col-md-2">
            @if($key==0) <label for="amount"> {{__('common.total_price')}} </label> @endif
            {!! Form::number('total_price[]',$value->qty*$value->price,['id'=>'total_price_'.$key,'class'=>'form-control input-number','placeholder'=>trans('common.total_price'), 'step'=>"any", 'required', 'readonly']); !!}
        </div>
        <div class="col-md-1">
            @if($key==0) <label for="description">{{__('common.description')}}  </label> @endif
    ...
    <div class="">
        <div class="col-md-4  pl-0">
            ...
            <div class="form-group">
                ...
                {!! Form::select('cash_or_bank_id', $banks, null,['class'=>'form-control select2 ','required', 'placeholder'=>trans('common.select_account_name')]); !!}
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                {!! Form::text('vehicle_number',null,['id'=>'vehicle_number','class'=>'form-control ',trans('common.enter_transport_vehicle_number')]); !!}
            </div>
            <div class="form-group">
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                {!! Form::number('total_bill',$modal->total_price,['id'=>'total_bill','class'=>'form-control input-number','placeholder'=>trans('common.enter_price'), 'step'=>"any", 'required', 'readonly']); !!}
            </div>
            <div class="form-group">
                <label for="due_advance" id="due_advance"> {{$modal->due_amount>0? trans('common.due'):trans('common.advance') }}</label>
                {!! Form::number('due_amount',$modal->due_amount>0? $modal->due_amount:$modal->advance_amount,['id'=>'total_due_adv','class'=>'form-control input-number','placeholder'=>trans('common.enter_price'), 'step'=>"any", 'required', 'readonly']); !!}
            </div>
            <div class="form-group">
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                {!! Form::number('transport_cost', $modal->transport_cost,['id'=>'transport_cost','class'=>'form-control input-number','placeholder'=>trans('common.enter_amount'), 'step'=>"any", 'required']); !!}
            </div>
            <div class="form-group">
                <label for="unload_cost"> {{__('common.unload_cost')}}</label>
                {!! Form::number('unload_cost', $modal->unload_cost, ['id'=>'unload_cost','class'=>'form-control input-number','placeholder'=>trans('common.enter_amount'), 'step'=>"any", 'required']); !!}
            </div>
            <div class="form-group">
                <label for="unload_cost"> {{__('common.bank_charge')}}</label>
                {!! Form::number('bank_charge', 0, ['id'=>'bank_charge','class'=>'form-control input-number','placeholder'=>trans('common.enter_amount'), 'step'=>"any", 'required']); !!}
            </div>
            <div class="form-group">
                <label for="amount"> {{__('common.total_amount')}}</label>
                {!! Form::number('total_amount',$modal->total_amount,['id'=>'total_amount','class'=>'form-control input-number','placeholder'=>trans('common.enter_price'), 'step'=>"any", 'required', 'readonly']); !!}
            </div>
            <div class="form-group">
                <label for="amount"> {{__('common.amount_to_pay')}} </label>
                {!! Form::number('amount_to_pay',$modal->amt_to_pay,['id'=>'amount_to_pay','class'=>'form-control input-number','placeholder'=>trans('common.enter_price'), 'step'=>"any", 'required', 'readonly']); !!}
            </div>
            <div class="form-group">
                <label for="amount"> {{__('common.paid_price')}}</label>
                {!! Form::number('paid_amount',$modal->paid_amount,['id'=>'paid_amount','class'=>'form-control input-number','placeholder'=>trans('common.enter_price'), 'step'=>"any", 'required']); !!}
            </div>
        ...
        $(function () {
            ...
            $(document).on('click', '.add-row', function () {
                ...
                var html = '<div class="box-body purchase-box ">' +
                    ...
                    '</div>'+
                    '<div class="col-md-1">' +
                    '<input placeholder=" {{__("common.price")}}"  min="0"  id="price_' + i + '" data-option="' + i + '" class="form-control price input-number" step="any" name="price[]"  type="number"/>' +
                    '</div>' +
                    '<div class="col-md-1">' +
                    '<input placeholder="{{__("common.total_price")}}" min="0"  id="price_' + i + '" data-option="' + i + '" class="form-control input-number" step="any" name="total_price[]"  type="number"/>' +
                    '</div>' +
                    '<div class="col-md-1">' +
                    '{!! Form::text('description[]',null,["id"=>"description","class"=>"form-control" ,"placeholder"=>trans("common.enter_description"),"height"=>"70px !important" ]); !!}' +
                    '</div>' +
