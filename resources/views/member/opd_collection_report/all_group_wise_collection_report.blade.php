...
$route = \Auth::user()->can(['member.specimen_wise.collection.list']) ? route('member.specimen_wise.collection.list') : '#';
...
$data['data'] = [
    'name' => 'Specimen Wise Collection List',
    'title' => 'Specimen Wise Collection',
    'heading' => 'Specimen Wise Collection',
];
                        ...
                        <div class="row pl-5 pr-5 my-3">
                            <div class="col-12 table-responsive">
                                <p style="text-align:center;font-size:14px;color:#000;font-weight:bold;"> Report of OPD
                                    Test Group Wise
                                    Collection Between {{ $from_date }} to {{ $to_date }}
                                </p>
                            ...
                            @endphp
                                @foreach ($data_is as $key => $speci)
                                    ...
                                    <table id="test_table"
                                        ...
                                        <tbody>
                                            ...
                                            @endphp
                                            @foreach ($speci ?? [] as $list)

                                                @php
                                                    $sum = $sum + count($list) * $list[0]->price;
                                                    $total_revenue = $total_revenue + count($list) * $list[0]->price;

                                                @endphp

                                                <tr>
                                                    <td align="left">{{ ++$i }}</td>
                                                    <td align="left">{{ @$list[0]->subTestGroup->title }}</td>
                                                    <td align="left">{{ count($list) }}</td>
                                                    <td align="left">{{ @$list[0]->price }}</td>
                                        ...
                                        <tfoot>
                                            <tr>
                                            <tr>
                                                <td style="font-size:16px;font-weight:bold" colspan="4" align="right">
                                                    Total:</td>
                               ...
                               <table id="test_table"
                                ...
                                <tbody>
                                    <tr>
                                        <td align="right" style="font-weight:bold;width: 86%;">
                                            <span style="font-size:16px">Total Revenue :</span>
                                        </td>

                                        <td>
                                            <p style="font-weight:bold;text-align:left">{{ $total_revenue }}</p>
                                        </td>
