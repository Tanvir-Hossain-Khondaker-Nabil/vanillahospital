...
$data['data'] = [
    ...
    'heading' => trans('purchase.load_to_warehouse'),
];
...
@section('contents')
    ...
    <div class="box box-default">
        ...
        <div class="box-header with-border">
            <h3 class="box-title">{{__('common.memo_no')}}: {{$model->memo_no}}</h3>
        </div>

        {!! Form::model($model, ['route' => ['member.warehouse.load.store'],  'method' => 'post', 'role'=>'form', 'id'=>"load_purchase"]) !!}

        <input type="hidden" value="{{ $model->id }}" name="purchase_id">
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="inputPassword" >{{__('common.purchase_date')}}</label>
                ...
                <div class="col-md-4">
                    <div class="form-group ">
                        <label for="inputPassword" >{{__("common.supplier_name")}}</label>

                        {!! Form::text('name',$model->supplier->name,['class'=>'form-control','readonly']); !!}
                    </div>
                </div>
        ...
        <div class="row">
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-sx-12  new-table-responsive text-center">
                ...
                <h4>{{__('common.load_itemwise_load_bulk')}} <label class="btn btn-bitbucket"> {{__('common.load')}}  <i class="fa fa-plus"></i> </label>  </h4>
                <input type="hidden" id="total_products" value="{{count($model->purchase_details)}}">
                ...
                <table class="table table-responsive table-striped">
                    ...
                    @php
                        $i=0;
                    @endphp
                    @foreach($model->purchase_details as $key => $purchase)
                        @php
                            $store_qty = $purchase->warehouseItemQty($purchase->id, $purchase->item_id);
                            $warehouse = $purchase->warehouse($purchase->id, $purchase->item_id);

                        $qty = $store_qty>0? "Loaded: ".$store_qty.", Due: "  : "";
                        $qty .= ($purchase->qty-$store_qty);
                        @endphp


                        @if($store_qty<$purchase->qty)
                            ...
                            @if($key==0)
                                ...
                                <tr>
                                    ...
                                    <th   class="text-center">{{__('common.last_load_qty')}}</th>
                                    ...
                                    <th width="500px" >
                                        <table  width="100%" >
                                            <tr>
                                                ...
                                                <th  width="125px"   class="text-left">{{__('common.load_qty')}}</th>
                                            </tr>
                                        </table>
                                    </th>
                                    <th></th>
                                ...
                                <tr class="item-row">
                                    <td class="item-name" width="300px">
                                        {!! Form::hidden('purchase_details[]', $purchase->id,['class'=>'form-control']); !!}
                                        {!! Form::hidden('product_id[]', $purchase->item_id,['class'=>'form-control']); !!}
                                        {!! Form::text('product[]',$purchase->item->item_name,['class'=>'form-control', 'disabled']); !!}
                                    </td>
                                    <td width="80px">
                                        {!! Form::text('unit[]',$purchase->unit,['class'=>'form-control text-center', 'disabled']); !!}
                                    </td>
                                    <td>{!! Form::text('last_load_qty[]',$purchase->warehouseLastUnload($purchase->item_id),['class'=>'form-control text-center', 'readonly']); !!}</td>
                                    <td>
                                        {!! Form::text('qty[]',$qty,['class'=>'form-control text-center ',  'step'=>"any", 'required', 'readonly']); !!}
                                        {!! Form::hidden('u_qty[]',$purchase->qty-$store_qty,['id'=>'qty_'.$key,'class'=>'form-control text-center ',  'step'=>"any", 'required', 'readonly']); !!}
                                    </td>

                                    {{--<td class="item-name" width="500px" colspan="3">--}}
                                    {{--<table width="100%" id="warehouse_{{$key}}">--}}
                                    {{--<tr>--}}
                                    {{--<td width="250px"> {!! Form::select('warehouse_id['.$purchase->item_id.'][]', $warehouses, null,[ 'class'=>'form-control select2','required', 'placeholder'=>'Select Warehouse']); !!}</td>--}}
                                    {{--<td width="150px" class="px-3">{!! Form::number('unload_qty['.$purchase->item_id.'][]',$purchase->qty-$store_qty,['class'=>'form-control ',  'step'=>"any", 'required', 'max'=>$purchase->qty-$store_qty]); !!}</td>--}}
                                    {{--<td><a class="btn btn-primary btn-sm multi-row" href="javascript:void(0)" data-target="{{$purchase->item_id}}" data-content="{{$key}}"> <i class="fa fa-clone"></i> </a></td>--}}
                                    {{--</tr>--}}
                                    {{--</table>--}}
                                    {{--</td>--}}
                                    ...
                                    <td class="item-name" width="500px">
                                        <table width="100%" id="warehouse_{{$key}}">
                                            <tr>
                                                <td width="250px"> {!! Form::select('warehouse_id['.$purchase->item_id.'][]', $warehouses, null,[ 'class'=>'form-control select2 warehouse-product','required', 'placeholder'=>'Select Warehouse',  'data-target'=>$purchase->item_id.$key, 'data-item'=>$purchase->item_id ]); !!}</td>
                                                <td width="125px" class="px-3">{!! Form::text('available_qty[]',null,['class'=>'form-control text-center', 'disabled', 'id'=>"available_qty_".$purchase->item_id.$key ]); !!}</td>
                                                <td width="125px" class="px-3">{!! Form::number('unload_qty['.$purchase->item_id.'][]',$purchase->qty-$store_qty,['class'=>'form-control  text-center',  'step'=>"any", 'required', 'max'=>$purchase->qty-$store_qty]); !!}</td>
                                                <td><a class="btn btn-primary btn-sm multi-row" href="javascript:void(0)" data-target="{{$purchase->item_id}}" data-content="{{$key}}"> <i class="fa fa-clone"></i> </a></td>
                                            </tr>
                                        </table>
                                    </td>
            ...
            <div style="margin-top: 20px; " class="row">
                ...
                <div class="col-lg-12 col-md-12 "   style="max-width: 100%; flex: 100%;">
                    <table class="new-table-3 pull-right" style="margin-top: 20px; margin-bottom: 20px"  >
                        <tr>
                            <td>
                                <button style="width: 150px !important; margin-right: 30px;" type="button" class="btn btn-block btn-primary load-submit">{{__('common.complete_load')}} </button>
                            </td>
                        </tr>
                    </table>
                </div>
            ...
            <div style="margin-top: 20px; " class="row">
                ...
                <div class="col-lg-12 col-md-12 "   style="max-width: 100%; ">
                    <div class="alert alert-success text-center">
                        <h4 class="mb-0">  {{__('common.load_completed')}}</h4>
                    </div>
                </div>
            </div>
