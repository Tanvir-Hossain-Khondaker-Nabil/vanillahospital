...
$data['data'] = [
    'name' => 'Procurements/Requisition Budget',
    'title'=>'Edit Procurements/Requisition Budget',
    'heading' => 'Edit Procurements/Requisition Budget',
];
    ...
    <div class="col-md-12">
        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">Edit Procurements/Requisition Budget</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->

            {!! Form::open(['route' => ['member.procurements.update', $procurement], 'method' => 'put', 'role'=>'form', 'id' => "pro-update" ]) !!}
                ...
                <div class="box-body">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            {!! Form::select('month',$months,$procurement->month,['id'=>'month','class'=>'form-control select2','placeholder'=>'Please select']); !!}
                        </div>
                        <div class="form-group">
                            <label for="department_id">Department</label>
                            {!! Form::select('department_id',$departments, $procurement->department_id,['id'=>'department_id','class'=>'form-control select2','placeholder'=>'Please select']); !!}
                        </div>
                        <hr>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <table width="100%" class="table table-bordered table-hover">
                                ...
                                <tbody id="appendBudget">
                                    @foreach ($procurement->procurement_details->where('status',0) as $key => $val)
                                    <tr  data-department = '{{$val->department_id}}' data-product = '{{$val->item_id}}' class="my-2 list-budget target-{{$key}}">
                                        <td>{{$val->department->name}}</td>
                                        <td>{{$val->item->item_name}}</td>
                                        <td>{{round($val->qty,0)}}</td>
                                        <td>{{$val->amount}}</td>
                                        <td>
                                            <input type='hidden' name='deparment_id[]' value='{{$val->department_id}}'>
                                            <input type='hidden' name='item_id[]' value='{{$val->item_id}}'>
                                            <input type='hidden' name='qty[]' value='{{$val->qty}}'>
                                            <input type='hidden' name='total_amount[]' value='{{$val->amount}}'>
                                            <span onclick='onDeleteBudget(".target-{{$key}}")' class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></span>
                                        </td>
                                    </tr>
                                    @endforeach
                    ...
                    <div class="box-footer">
                        <div class="col-md-12 text-right">
                            <button type="button" onclick="onSubmit()" id="load-submit" class="btn btn-primary">Update</button>
                        </div>
                    </div>
