...
$data['breadcrumb'] = [
    ...
    [
        'name' => 'Sale Report By Customer',
    ],
];

$data['data'] = [
    'name' => 'Sale Report By Customer',
    'title'=> 'Sale Report By Customer',
    'heading' => trans('common.sale_report_by_customer'),
];
...
@section('contents')
    ...
    <div class="row">
        <div class="col-xs-12">
            ...
            {!! Form::open(['route' => 'member.report.customer_sale','method' => 'GET', 'role'=>'form' ]) !!}
            <!-- /.box-header -->
                <div class="box-body">
                    <div class="row">
            ...
            <div class="box">
                <div class="box-header with-border">
                    <h3 class="box-title"> {{__('common.sale_report_by_customer')}}</h3>
                    ...
                    <div class="col-lg-12">
                        <table class="report-table table table-striped" id="dataTable">
                            ...
                            @foreach($sales as $key => $value)
                                @php
                                    $saledate = db_date_month_year_format($value->sale->date);
                                @endphp
                                ...
                                @if($value->sale_status == "return")
                                    @php
                                        $return = $value->sale_returns($value->sale_id, $value->item_id)
                                    @endphp
                                    @foreach($return as $value2)
                                        <tr>
                                            <td>{{ $key = $key+1 }}</td>
                                            <td>{{ db_date_month_year_format($value2->sales->date) }}</td>
                                            <td class="text-left">{{  $value2->sales->sale_code }}</td>
                                            <td>{{  $value2->sales->id }}</td>
                                            <td>Sale Return</td>
                                            <td class="text-left">{{ $value2->sales->due > 0 ? $value2->sales->customer ? $value2->sales->customer->name : "" : "Cash" }}</td>
                                            <td></td>
                                            <td class="text-left">{{ $value2->item->item_name }}</td>
                                            <td>{{ $value2->unit }}</td>
                                            <td class="text-right padding-right-20">({{ $value2->return_qty }})</td>
                                        ...
                                        @php
                                            $sale_total_price -= $value2->return_qty*$value2->return_price;
                                ...
                                @php
                                    ...
                                    $sale_total_price += $value->qty*$value->price;
