...
<body>
        ...
        <table class="farhana-table-1">
            <tbody>
                <tr style="text-align: center">
                    ...
                    <td style="">
                        <h1 class="" style="margin-top: 2px; font-size: 20px;margin-left: 5px;">
                            {{ $company_name }} </h1>

                        <p>আপনার সু-চিকিৎসাই আমাদের ঐক্যবদ্ধ প্রচেষ্টা </p>


                        <p>{{ $company_address }}
                        </p>

                        <p>Mobile Number : {{ $company_phone }}</p>
                    </td>
        ...
        <tbody>
            <tr>
                <td colspan="2"><b>Bill No: </b><label
                        id="invoice">{{ $patient_info->outDoorRegistration->opd_id }}</label></td>
                <td><b>Date: </b><label id="date_time">4/6/2024 9:56:04 AM</label></td>
            </tr>
            <tr>

                <td><b>Sex: </b><label id="gender">{{ $patient_info->outDoorRegistration->gender }}</label></td>
                <td class="text-center"><b>Age: </b><label
                        id="age">{{ $patient_info->outDoorRegistration->age }}</label>
                </td>
                <td><b>Mob: </b><label id="phone_no">{{ $patient_info->outDoorRegistration->phone }}</label></td>
            </tr>
            <tr>

                <td style="text-align: left"><b>Patient Name: </b><label
                        id="patient_name">{{ $patient_info->outDoorRegistration->patient_name }}</label></td>
                <td><b>Patient Address: </b><label
                        id="patient_address">{{ $patient_info->outDoorRegistration->address }}</label></td>

            </tr>

            <tr>

                @if ($patient_info->outDoorRegistration->doctor_id == 0)
                    <td colspan="3" class="doctor-name"><b>Dr. Name: </b><label> Self</label></td>
                @else
                    <td colspan="3" class="doctor-name"><b>Dr. Name: </b><label
                            id="ref_by">{{ $patient_info->outDoorRegistration->doctor ? $patient_info->outDoorRegistration->doctor->name : '' }}
                            ({{ $patient_info->outDoorRegistration->doctor ? $patient_info->outDoorRegistration->doctor->degree : '' }})
                        </label></td>
                @endif

            </tr>
            <tr>
                @if ($patient_info->outDoorRegistration->ref_doctor_id == 0)
                    <td colspan="3" class="doctor-name"><b>Ref Dr. Name: </b><label> Self</label></td>
                @else
                    <td colspan="3" class="doctor-name"><b>Ref Dr. Name: </b><label
                            id="quack_by">{{ $patient_info->outDoorRegistration->refDoctor ? $patient_info->outDoorRegistration->refDoctor->name : '' }}
                            ({{ $patient_info->outDoorRegistration->refDoctor ? $patient_info->outDoorRegistration->refDoctor->degree : '' }})</label>
                    </td>
                @endif
    ...
    @foreach ($ids as $key => $test_id)
        @php
            $pathologyReport = pathologyFinalReport($test_id);
            $technologist = technologist($pathologyReport->sub_test_group_id);
            //    dd($pathologyReport,  $technologist);
        @endphp

        @if ($pathologyReport)
            <p>{!! $pathologyReport->description !!}</p>
        @endif
