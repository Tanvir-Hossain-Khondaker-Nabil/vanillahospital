...
<script type="text/javascript">
    $(function () {
        ...
        $(document).on('keyup',' .total_price', function(e) {
            ...
            price_calculate("total_price");

            var $divID = $(this).data('option');
            var stock = $("#stock_" + $divID).val();
            var qtyCheck = $("#qty_" + $divID).val();
            if (parseFloat(stock) < qtyCheck) {
                $("#qty_" + $divID).val(stock);
        ...
        function price_calculate(type){
            ...
            for(var i = 0; i<=$tr.find('tr').length; i++) {
                ...
                if(calType == "qty")
                {
                    ...
                    var qty = total_price/price;
                    $tr.find('tr:nth-child('+i+') td:nth-child(4) input').val(qty.toFixed(2));
                }
