<?php
/**
 * Created by PhpStorm.
 * User: Mobarok Hossen
 * Date: 1/1/2018
 * Time: 11:37 PM
 */

$route = \Auth::user()->can(['member.opd.test_discount_other_report']) ? route('member.opd.test_discount_other_report') : '#';
$home1 = \Auth::user()->can(['member.dashboard']) ? route('member.dashboard') : '#';

$data['breadcrumb'] = [
    [
        'name' => 'Home',
        'href' => $home1,
        'icon' => 'fa fa-home',
    ],
    [
        'name' => 'OPD Test Discount Other Report',
        'href' => $route,
    ],
    [
        'name' => 'Index',
    ],
];

$data['data'] = [
    'name' => 'OPD Test Discount Other Report',
    'title' => 'OPD Test Discount Other Report',
    'heading' => 'OPD Test Discount Other Report',
];

?>

@extends('layouts.back-end.master', $data)

@push('styles')
    <link rel="stylesheet" href="{{ asset('public/adminLTE/bower_components/select2/dist/css/select2.min.css') }}">
@endpush

@section('contents')
    <div class="row">
        <div class="card my-3 no-b">
            <div class="card-body">
                <div class="container">
                    <div class="invoice white shadow">
                        <div class="row pl-5 pr-5">
                            <div class="col-md-3">
                                @if (@$company_logo)
                                    <img height="120px" width="120px" src="{{ $company_logo }}">
                                @endif
                            </div>
                            <div class="col-md-9">

                                <h1 style="text-align:center;font-size:26px;color:black;font-weight:bold;">
                                    {{ $company_name }}
                                </h1>
                                <h1 style="text-align:center;font-size:20px;color:black;">
                                    {{ $company_address }}
                                </h1>
                                <p style="text-align:center;font-size:18px;color:black;margin-top:-8px;">

                                    Mobile: {{ $company_phone }}
                                </p>
                            </div>

                            <div class="col-md-12" style="border-bottom:#000 solid 1px">
                            </div>

                        </div>

                        <div class="row pl-5 pr-5 my-3">
                            <div class="col-12 table-responsive">
                                <p style="text-align:center;font-size:14px;color:#000;font-weight:bold;"> Report of OPD test
                                    discount for others between {{ $from_date }} to {{ $to_date }} </p>

                                {{-- {{dd($data_is)}} --}}
                                @php

                                    $total_sum = 0;

                                @endphp
                                @foreach ($data_is as $key => $speci)
                                    <h5 align="center"> Notes: {{ $key }} </h5>
                                    <table id="test_table"
                                        class="table table-bordered table-hover test_table_report test_table_report">
                                        <thead>
                                            <tr>
                                                <th>SL</th>
                                                <th>Test Name</th>
                                                <th>Price</th>
                                                <th>Discount</th>
                                                <th>Net Amount</th>
                                            </tr>
                                        </thead>

                                        <tbody>
                                            @php
                                                $i = 0;
                                                $sum = 0;
                                                $total_discount = 0;
                                                // $discount = 0;
                                                // $total_revenue = 0;
                                            @endphp
                                            @foreach ($speci ?? [] as $per_data)
                                                @foreach ($per_data->outdoorPatientTest ?? [] as $list)
                                                    @php
                                                        $sum = $sum + $list->price;
                                                        $total_discount = $total_discount + $list->discount;
                                                        $total_sum = $total_sum + $list->price;
                                                    @endphp
                                                    <tr>
                                                        <td align="left">{{ ++$i }}</td>
                                                        <td align="left">{{ $list->subTestGroup->title }}</td>
                                                        <td align="left">{{ $list->price }}</td>
                                                        <td align="left">{{ $list->discount }}</td>
                                                        <td align="left">{{ $list->net_amount }}</td>
                                                    </tr>
                                                @endforeach
                                            @endforeach

                                            @php

                                                // $total_revenue = $total_revenue + $sum;
                                            @endphp
                                        </tbody>
                                        <tfoot>
                                            <tr>
                                                <td style="font-size:16px;font-weight:bold" colspan="2" align="right">
                                                    Total:
                                                </td>
                                                <td style="font-weight:bold"> {{ $sum }}</td>


                                                <td style="font-weight:bold"> {{ $total_discount }}</td>

                                            </tr>
                                        </tfoot>

                                    </table>
                                @endforeach




                                <table id="test_table"
                                    class="table table-bordered table-hover test_table_report test_table_report">
                                    <tbody>
                                        <tr>
                                            <td align="right" style="font-weight:bold;width: 86%;">
                                                <span style="font-size:16px">Total Revenue :</span>
                                            </td>

                                            <td>
                                                <p style="font-weight:bold;text-align:left">{{ $total_sum }}</p>
                                            </td>

                                        </tr>
                                        <tr>
                                            <td align="right" style="font-weight:bold;width: 80%;font-size:16px"><span
                                                    style="font-size:16px">Total Discount :</span></td>

                                            <td>
                                                <p style="font-weight:bold;text-align:left">{{ $discount }}</p>
                                            </td>

                                        </tr>
                                        <tr>
                                            <td align="right" style="font-weight:bold;width: 80%;font-size:16px"><span
                                                    style="font-size:16px">Total Gross :</span></td>

                                            <td>
                                                <p style="font-weight:bold;text-align:left">
                                                    {{ $total_sum - $discount }}</p>
                                            </td>

                                        </tr>
                                    </tbody>
                                </table>

                            </div>
                            <!-- /.col -->
                        </div>

                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection


@push('scripts')
    <script src="{{ asset('public/adminLTE/bower_components/select2/dist/js/select2.full.min.js') }}"></script>

    <script>
        $(function() {
            $('.select2').select2();

        });

        $('#from_date').datepicker({

            // "format": 'yyyy-mm-dd',
            "format": 'mm/dd/yyyy',
            "showTime": true,
            // "startDate": "+0d",
        });

        $('#to_date').datepicker({

            // "format": 'yyyy-mm-dd',
            "format": 'mm/dd/yyyy',
            "showTime": true,
            // "startDate": "+0d",
        });

        $(function() {
            $("#vanilla-table1").DataTable({
                // "lengthMenu":[ 3,4 ],
                "searching": true,
            });
            $("#vanilla-table2").DataTable({

                "searching": true,
            });
            $("#vanilla-table3").DataTable({

                "searching": true,
            });

        });
    </script>

    <script>
        setTimeout(function() {
            window.print();

        }, 500);
    </script>
    </script>
@endpush
