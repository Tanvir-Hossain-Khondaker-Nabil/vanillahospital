...
$data['data'] = [
    'name' => ' Quotation Purchase ',
    'title'=> ' Quotation Purchase with Multi supplier',
    'heading' => ' Quotation Purchase with Multi Supplier',
];
        ...
        <!-- general form elements -->
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">  Quotation Purchase</h3>
                </div>


                {!! Form::open(['route' => 'member.purchase_quotations.multi_supplier_based_store','method' => 'POST', 'files'=>true, 'id'=>'purchase_form', 'role'=>'form' ]) !!}

                {!! Form::hidden('quotation_id', $modal->id, [ 'class'=>'form-control','required']) !!}
                <div class="box-body">
                    ...
                    <div class="box">
                        ...
                        @foreach( $modal->quotation_details as $key => $value)
                            ...
                            @if($value->qty-$value->purchase_qty>0)
                                @php
                                $hasItem++;
                                @endphp
                            <div class="box-body  purchase-quotation">

                                <div class="col-md-3 pr-2">
                                    @if($key==0) <label for="product_id">Item Name </label> @endif
                                    {!! Form::hidden('quotation_details_id[]', $value->id, [ 'class'=>'form-control','required']); !!}
                                    {!! Form::select('product_id[]', $products, $value->item_id,['id'=>'product_id_'.$key, 'data-option'=>$key, 'class'=>'form-control select2 item-name','required', 'placeholder'=>'Select Item Name']); !!}
                                </div>
                                ...
                                <div class="col-md-1 px-2">
                                    ...
                                    {!! Form::number('qty[]',$value->qty-$value->purchase_qty,['id'=>'qty_'.$key,'class'=>'form-control qty', 'required', 'step'=>"any"]); !!}
                                </div>
                                <div class="col-md-1  px-2">
                                ...
                                <div class="col-md-1 px-0">
                                    ...
                                    {!! Form::number('total_price[]',$value->qty*$value->price,['id'=>'total_price_'.$key,'class'=>'form-control input-number total_price','placeholder'=>'Total Price', 'step'=>"any", 'required']); !!}
                                </div>
                                ...
                                <div class="col-md-2 pl-2">
                                    @if($key==0) <label for="description">Select Supplier  </label> @endif
                                ...
                                <div class="col-md-1 text-center" style=" padding-top:{{ $loop->last  ?  "10px" : ($loop->first ? "30px" :"10px") }}">
                                    @if($loop->last)
                                        {{--<a class="btn btn-success add-row btn-xs  margin-top-3" href="#" ><i class="fa fa-plus-circle"></i> </a>--}}
                                        <a href="#" class="btn-text-danger margin-top-23 delete-field"><i class="fa fa-close"></i></a>
                                    @else
                                        <a href="#" class="btn-text-danger margin-top-23 delete-field"><i class="fa fa-close"></i></a>
                                    @endif
                                </div>
                        ...
                        @if($hasItem==0)
                            ...
                            <div class="box-body  purchase-quotation">
                                ...
                                <div class="col-md-12 pr-2">
                                    <h4 class="text-danger text-center"> No Items Have To Purchase from This Quotation </h4>
                                </div>
                            </div>
                        @endif
                    </div>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $(document).on('click', '.add-row', function () {
                ...
                if($div.find('div :first-child').find('select').val() == "" || $div.find('div :nth-child(3)').find('input').val() == "" || $div.find('div :nth-child(5)').find('input').val() == "")
                {
                ...
                var html = '<div class="box-body purchase-quotation">' +
                    '<div class="col-md-3">' +
                    ...
                    '</div>'+
                    '<div class="col-md-2 ">' +
                    '<input placeholder=" Price"  min="0"  id="price_' + i + '" data-option="' + i + '" class="form-control price input-number" step="any" name="price[]"  type="number" readonly/>' +
                    '</div>' +
            ...
            $(document).on('keyup','.qty, .price, .total_price', function(e) {
                e.preventDefault();
            ...
            function price_calculate(){
                var total_bill = 0;
                for(var i = 1; i<=$('.purchase-quotation').length; i++) {

                    var qty = $('.purchase-quotation:nth-child(' + i + ') div:nth-child(3) input').val();
                    var total_price = $('.purchase-quotation:nth-child(' + i + ') div:nth-child(5) input').val();

                    // var price = $('.purchase-quotation:nth-child(' + i + ') div:nth-child(4) input').val();
                    // price =  price == undefined || price == "" ? 0 : parseFloat(price);
                    // var total_price = parseFloat(qty*price);
                    // $('.purchase-quotation:nth-child(' + i + ') div:nth-child(5) input').val(total_price);
                    ...
                    $('.purchase-quotation:nth-child(' + i + ') div:nth-child(4) input').val(price);
