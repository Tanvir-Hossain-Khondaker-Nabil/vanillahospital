...
<div class="box-group head-trans">
    ...
    <div class="col-md-3">
        <label for="cash_or_bank_id"> {{ $type }} From </label>
        <select required name="transaction_form" id='cash_or_bank_id' class='form-control select2' placeholder='Select Account'>
            <option value="">Please select </option>
            @foreach($transaction_categories as $key=> $value)
{{--                data-action="{{ $value->account_type_id }}" data-option="{{ $value->current_balance }}"--}}
                <option value="{{ $key }}" {{ $key == $transaction_form ? "selected='selected'" : '' }}>{{ $value }}</option>
            @endforeach
        </select>
    ...
    <div class="col-md-3">
        <label for="payer"> {{ $type=='Payment' ? 'Received Person': 'Sender' }} Type </label>
        {!! Form::select('payer_type', ['customer'=>'Customer','supplier'=>'Supplier','both'=>'Customer & Supplier'], null,['id'=>'payer','class'=>'form-control ','placeholder'=>'Please Select']); !!}
    </div>

    <div class="col-md-3">
        <label for="supplier_id"> {{ $type=='Payment' ? 'Received By': 'Sender By' }}  </label>
        {!! Form::select('supplier_id', $sharers , $transactions->supplier_id,['id'=>'supplier_id','class'=>'form-control select2','placeholder'=>'Please Select']); !!}
    </div>
</div>
...
<div class="box main-box">
    ...
    <div class="box-body">
        <div class="col-md-1">
            <label for="category_id">{{ $type == "Expense" ? "Account" : "" }} Code </label>
            {!! Form::text('account_type[]', $value->account_type_id,['id'=>'account_id_0', 'data-option'=>'0','class'=>'form-control acc_id', 'placeholder'=>'Select Account Code']); !!}
        </div>
        <div class="col-md-3">
            <label for="category_id">{{ $type == "Expense" ? "Account To" : $type." To" }} </label>
            {!! Form::select('account_type_id[]', $transaction_categories, $value->account_type_id,['id'=>'account_name_0', 'data-option'=>'0', 'class'=>'form-control acc_name select2 acc_name','required', 'placeholder'=>'Please Select']); !!}
        ...
        <div class="col-md-2" >
            ...
            {!! Form::select('payment_method[]', $payment_methods, $value->payment_method_id,['id'=>'payment_method_0', 'data-option'=>'0', 'class'=>'form-control select2 payment_method_change','required']); !!}
            <div class="payment_method_addition">
            </div>
        </div>
        <div class="col-md-1">
            <label for="amount"> Amount </label>
            {!! Form::text('amount[]',$value->amount,['id'=>'amount','class'=>'form-control input-number','placeholder'=>'Enter Amount', 'step'=>"0.000", 'required']); !!}
        </div>
        <div class="col-md-3">
